...
const TwoWorksVideo = dynamic(() => import("./components/TwoWorksVideo"), {
  ssr: false,
  loading: () => (
    <div className="text-center text-slate-400">Загружаем видео…</div>
  ),
});
const Showcase = dynamic(() => import("./components/Showcase"), {
  ssr: false,
  loading: () => (
    <div className="text-center text-slate-400">Загружаем кейсы…</div>
  ),
});
const Songs = dynamic(() => import("./components/Songs"), {
  ssr: false,
  loading: () => (
    <div className="text-center text-slate-400">Загружаем песни…</div>
  ),
});
const Scenes = dynamic(() => import("./components/Scenes"), {
  ssr: false,
  loading: () => (
    <div className="text-center text-slate-400">Загружаем процесс…</div>
  ),
});
...
export default function Page() {
  ...
  const asset = (path: string) => (siteUrl ? `${siteUrl}${path}` : path);
  ...
  const schema = [
    {
      ...
      name: "AI Memories",
      url: siteUrl || "/",
      logo: asset("/images/og-cover.svg"),
      description:
